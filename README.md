...
- 🗑️ **Delete & Recover Messages**: Press `d` or `Delete` to move messages to the Trash (Deleted Items folder) on Outlook. Press `D` (Capital D) to delete the entire conversation thread of the selected message at once (with confirmation). Press `U` (Capital U) to recover/undelete a message from the Deleted Items folder back to the Inbox. Press `E` (Capital E) in the Folders pane to empty all messages from the selected folder (with confirmation; Inbox and Sent Items are protected by default via `protected_folders`).
