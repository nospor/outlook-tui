...
- ✉️ **Send & Compose Mail**: Press `n` to open a full compose screen to draft and send new emails. With SQLite caching enabled, it features interactive autocomplete for the `To` field (use `Up`/`Down` to navigate suggestions, `Enter` to select/autocomplete, and `Esc` to close suggestions popup).
